...
let source_stage_name = '';
let target_stage_name = '';
let item_id = '';
...
function dragula (initialContainers, options) {
  ...
  function canStart (item) {
    ...
    source_stage_name =source.getAttribute("id");

    console.log("soursource_stage_namece");
    console.log(source_stage_name);
    if (!source) {
      return;
    }
    if (o.invalid(item, handle)) {
...
console.log('_lastDropTarget');
console.log(_lastDropTarget);

if(_lastDropTarget !== null){
    target_stage_name =_lastDropTarget.getAttribute("id");
console.log('target_stage_name');
console.log(target_stage_name);
console.log('source_stage_name');
console.log(source_stage_name);
        ...
        if(target_stage_name =='backlog'){
          target_num=1;
        }else if(target_stage_name =='prepare'){
          target_num=2;
        }else if(target_stage_name =='progress'){
          target_num=3;
        }else if(target_stage_name =='check'){
          target_num=4;
        }else if(target_stage_name =='completed'){
          target_num=5;
        }
    ...
    let ids = [];
    const parent = document.getElementById(target_stage_name);
    Array.from(parent.children).forEach(child => {
      ids.push(child.id);
    });


    changeStage(item_id,target_num);
    changeOrder(ids,target_num);
  ...
  // ステージの変更処理
  function changeStage(id,target_num) {
    ...
    console.log("data_obj_changeStage");
    console.log(data_obj);
    fetch('/update-stage/', { // 第1引数に送り先
        method: 'POST', // メソッド指定
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': getCookie('csrftoken') // クッキーからCSRFトークンを取得
        }, // jsonを指定
  ...
  // 順番の変更処理
  function changeOrder(ids,target_num) {
    console.log("ids");
    console.log(ids);
    var data_obj = {
      id: ids,
    ...
    console.log("data_obj_changeOrder");
    console.log(data_obj);
    fetch('/update-order/', { // 第1引数に送り先
        method: 'POST', // メソッド指定
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': getCookie('csrftoken') // クッキーからCSRFトークンを取得
